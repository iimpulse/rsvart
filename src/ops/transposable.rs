use crate::{Contiged, Strand, Unit, Located, Stranded};
...
mod test {
    ...
    fn test_start_on_strand(){
        ...
        let test_region = TestRegion { strand: Strand::Reverse, start: 32, end: 200, contig };
        assert_eq!(test_region.start_on_strand(Strand::Reverse), *test_region.start());
    ...
    fn test_end_on_strand(){
        let contig = get_test_contig();
        let test_region = TestRegion { strand: Strand::Reverse, start: 32, end: 200, contig };
        assert_eq!(test_region.end_on_strand(Strand::Reverse), *test_region.end());
